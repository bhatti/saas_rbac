...
impl<'a, 'r> FromRequest<'a, 'r> for AssociationForm {
    ...
    fn from_request(req: &'a Request<'r>) -> request::Outcome<Self, Self::Error> {
        ...
        if !form.effective_at.is_empty() && form.parse(form.effective_at.as_str()).is_err() {
            return Failure((Status::BadRequest, "effective_at date must be of in format YYY-MM-DDTHH:mm:SS".to_string()));
        }
        if !form.expired_at.is_empty() && form.parse(form.expired_at.as_str()).is_err() {
            return Failure((Status::BadRequest, "expired_at date must be of in format YYY-MM-DDTHH:mm:SS".to_string()));
        }
        Success(form)
    }
}
